	...
	<form method="get" action="scorecard_list"> <%-- 검색기능 폼 추가 --%>
		<table class="boardlist" id="board_sclist">
			...
			<tr>
				...
				<input type="button" class="CheckBtn" id="CheckBtn_sc" value="글쓰기" onclick="location='scorecard_write?page=${page}'">
				</td>
			</tr>
		</table>
		<div class="page_control">
			...
			<c:if test="${(empty find_field) && (empty find_name) }"> <%-- 검색필드와 검색어가 없는 경우 --%>
				...
				<c:if test="${page > 1 }">
					<a href="scorecard_list?page=${page-1 }">[이전]</a>
				</c:if>
				...
				<c:forEach var="a" begin="${startpage }" end="${endpage }" step="1">
					...
					<c:if test="${a != page }">
						<a href="scorecard_list?page=${a }">${a}</a>
					</c:if>					
				</c:forEach>
				...
				<c:if test="${page < maxpage }">
					<a href="scorecard_list?page=${page+1 }">[다음]</a>
				</c:if>
			</c:if>
			...
			<c:if test="${(!empty find_field) || (!empty find_name) }">
				...
				<c:if test="${page > 1 }">
					<a href="scorecard_list?page=${page-1 }&find_field=${find_field}&find_name=${find_name}">[이전]</a>
				...
				<c:forEach var="a" begin="${startpage }" end="${endpage }" step="1">
					...
					<c:if test="${a != page }">
						<a href="scorecard_list?page=${a }&find_field=${find_field}&find_name=${find_name}">${a}</a>
					</c:if>					
				</c:forEach>
				<c:if test="${page >= maxpage }">
					[다음]
				...
				<c:if test="${page < maxpage }">
					<a href="scorecard_list?page=${page+1 }&find_field=${find_field}&find_name=${find_name}">[다음]</a>
				</c:if>
			</c:if>
  		</div>
  		</form>
